...
- **Not linked, but an `expectedFileName` is set** — before giving up, `scheduled-posts.ts` looks for a file with that *exact* name: first among this channel's already-synced-but-unclaimed `inbox/` folders, then (if not found there) live in the channel's Drive folder itself, downloading it directly if found. A match already sitting in `inbox/` links and **publishes immediately**, same as above. A fresh Drive download only **links** this run — it publishes on the *next* run instead, once the workflow's own commit step has actually pushed the downloaded file to GitHub (Buffer needs a working `raw.githubusercontent.com` URL, which doesn't exist yet for a file that's only on local disk). This is how you can name a file when planning the post, upload it to Drive whenever, and have it get picked up and posted automatically at the scheduled time without ever touching Content Queue again. Once it actually publishes, the same `_posted`-folder move described above applies here too.

...
The GitHub issue exists specifically to work around GitHub's own limitation on the failure email: it can only link to the run, not carry a custom body, so the actual message was previously only visible by digging into a log. The issue puts the real text (message, project, campaign, reason) somewhere you'd naturally see it via GitHub's normal issue notifications — close it once you've handled it. If issue creation itself fails for some reason (e.g. a permissions problem), it logs a `::warning::` and the run continues normally rather than blocking the rest of the notification.

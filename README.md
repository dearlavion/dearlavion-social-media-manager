...
- **Planned (ongoing campaigns)** — a lightweight content-planning checklist for the campaign currently selected in the Campaign picker above. Add a planned post directly here (a **stage** — `awareness`/`consideration`/`conversion`/`loyalty`, picking one fills its default guidance text which you can then edit — an optional **Caption** to publish with the image, and an optional **target date + time**; the campaign is already fixed by the picker, so there's nothing to choose there), edit or remove it, and toggle a **todo / done** prep-status chip tracking whether you've actually created that content yet — independent of whether it's synced or posted. The target-date dropdown is generated from the selected campaign's start/end dates (or the next 30 days if unset). **Caption** is stored on the slot itself and takes priority over a `caption.txt` file once this slot is linked to a synced post — it's only read at publish time, so editing it after the slot is already `queued`/`posted` changes what's stored but not anything already published.

...
3. **Post** (`.github/workflows/post.yml`, hourly, offset 15 min after sync): for every project, for each enabled channel, posts the oldest post folder in `inbox/<projectId>/<channelId>/` — single image, video, or carousel, inferred from how many media files are in it — via that channel's **posting tool** (`publisher` field — Buffer is the only one implemented today; see `automation/src/publishers/`), then moves the whole folder to `posted/<projectId>/<channelId>/` and updates `lastPostedAt`. Skips any folder reserved by a not-yet-due scheduled post (see below), and does nothing if the inbox is empty. **On success, the source file in Drive itself also gets moved** into a `_posted` subfolder (created automatically) of the channel's Drive folder — keeps it out of future syncs/lookups without renaming it, so it stays findable by its original name if you ever want it back. This needs Editor access on the Drive folder (see Setup above); if that's not granted, the post itself still succeeds — only the Drive-side move is skipped, logged as a warning in the run's log.
...
3. Share each Drive folder you'll use as an image source with that service account's email, as **Editor** (not just Viewer) — the automation needs write access to relocate a file into `_posted` after it publishes (see below), not just read it. One service account can be shared across as many projects/folders as you like.
...
2. Back on the Dashboard, **Load channels.json** for that project, then add/edit channels: a unique `id`, `platform` (label only, for your own reference), **Posting tool** (which backend actually publishes — only Buffer is implemented, see `automation/src/publishers/`), `driveFolderId`, and `bufferChannelId`. Once you've loaded a project, the Dashboard remembers it (`sessionStorage`, same tab) — refreshing the page re-runs the same loads automatically and lands you right back on it, no re-clicking, always fetching current data rather than a cached copy. **Campaigns** and **Content Queue** pick that same project up automatically too — once it's loaded, switching to either view loads its data right away, no separate "Load" click needed there either.

**Media type is set by how you organize the Drive folder**, not a separate field: a loose image or video file at the top level becomes a single-item post; a **subfolder** becomes a carousel post from every image/video inside it (up to 10 items, Instagram's cap). **Captions** come from either of two places, in priority order: a **Caption** set on a Content Queue planned post (stored on the campaign slot itself, takes effect once that slot links to a synced post) if the post is linked to one, otherwise a per-post `caption.txt` — add one directly to that post's folder on GitHub (`inbox/<projectId>/<channelId>/<postId>/caption.txt`, after `sync-drive` has created it — Drive sync only picks up image/video files, so this one's added on the GitHub side, not from Drive) — it moves along with the rest of that post once published. A post with neither publishes with an empty caption.
...
- **Planned & posted (ongoing campaigns)** — a lightweight content-planning checklist for the campaign currently selected in the Campaign picker above, showing **every slot for that channel regardless of status** — `planned`, `queued`, and `posted` all show here, each with a status badge (and a "✓ posted <timestamp>" badge once it's gone out), so the full history stays visible instead of disappearing once it publishes. Add a planned post directly here (a **stage** — `awareness`/`consideration`/`conversion`/`loyalty`, picking one fills its default guidance text which you can then edit — an optional **Caption** to publish with the image, and an optional **target date + time**; the campaign is already fixed by the picker, so there's nothing to choose there), edit or remove it (removal works at any status — it just deletes the tracking record, it doesn't touch the actual synced/posted media or unpublish anything from Buffer), and toggle a **todo / done** prep-status chip tracking whether you've actually created that content yet — independent of whether it's synced or posted. The target-date dropdown is generated from the selected campaign's start/end dates (or the next 30 days if unset). **Caption** is stored on the slot itself and takes priority over a `caption.txt` file once this slot is linked to a synced post — it's only read at publish time, so editing it after the slot is already `queued`/`posted` changes what's stored but not anything already published.
...
- **Not linked, but an `expectedFileName` is set** — before giving up, `scheduled-posts.ts` looks for a file with that *exact* name: **live in the channel's Drive folder first** (the authoritative source — picks up a freshly (re)uploaded file even if a stale same-named copy is already sitting in `inbox/`), downloading it directly if found; only if Drive doesn't have a match does it fall back to this channel's already-synced-but-unclaimed `inbox/` folders (e.g. from a regular hourly sync that ran before this slot came due). A match found already in `inbox/` links and **publishes immediately**, same as above. A fresh Drive download only **links** this run — it publishes on the *next* run instead, once the workflow's own commit step has actually pushed the downloaded file to GitHub (Buffer needs a working `raw.githubusercontent.com` URL, which doesn't exist yet for a file that's only on local disk). This is how you can name a file when planning the post, upload it to Drive whenever, and have it get picked up and posted automatically at the scheduled time without ever touching Content Queue again. Once it actually publishes, the same `_posted`-folder move described above applies here too. Either match is skipped (falling through to the other, or to the overdue notification) if it's already linked to a *different* slot — reusing the same file across multiple planned posts is fine, but only one slot can claim a given copy at a time.
...
The calendar and the selected day's card sit side by side (not stacked) so picking a day doesn't require scrolling. The calendar also shows a second dot for any day with Content Queue posts — every `ongoing` campaign's slot with a `targetDate`, **any status** (`planned`/`queued`/`posted`), across every project, loaded alongside reminders. Selecting a day lists them under a **Posts** heading, **grouped by campaign**, each with a status badge (green once posted) — so a day keeps showing what actually happened on it, not just what's still upcoming. It's read-only here (no editing or linking); use Content Queue or Campaigns for that.

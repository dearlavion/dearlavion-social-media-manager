...
- **Auto-completion**: when `post.yml` actually publishes a post whose folder is linked to a slot, `post.ts` flips that slot to `posted` and records the timestamp automatically — no manual bookkeeping once a post is linked. If a per-channel goal was set, the detail view shows actual posted count against it (e.g. "3/5 posted toward goal") — a progress readout only, nothing enforces it.

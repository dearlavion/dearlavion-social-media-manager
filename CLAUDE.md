...
**Multiple slots sharing one `linkedPostPath` breaks caption attribution (fixed)**: `scheduled-posts.ts`'s expected-filename lookup has two branches -- `findInInbox` (already-synced) correctly checks the `claimed` set (every `linkedPostPath` already in use by some slot, built by `claimedPaths()`) before matching, but the live-Drive-lookup branch (`findFileByName`) didn't, so re-finding a file already backing another slot (e.g. the same source reused across multiple planned posts, which happens easily since `postFolderName` is deterministic from the Drive file's own metadata) would silently link a *second* slot to the exact same path. Confirmed live: `post.ts`'s regular FIFO resolves which slot's caption to use via `findSlotByLinkedPath` (`post-helpers.ts`), which returned whichever slot happened to be *first* in array order for that path -- if that happened to be an older, caption-less slot while a *different*, actually-relevant slot (with a caption) shared the same path, the post went out with no caption regardless of what the right slot said. Now: the drive-lookup branch checks `claimed` before proceeding (skips relinking, logs it, rather than stealing), and `findSlotByLinkedPath` additionally prefers a still-`queued` match over an already-`posted` one as a defensive tiebreak for any pre-existing ambiguous data.
